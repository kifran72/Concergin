...
$req = $db->prepare("SELECT Prenom_Personne, Nom_Personne, Role_Personne FROM utilisateur WHERE ID_Personne=:id and MDP_Personne=:mdp");
...
if (count($resultat) == 0) {
    $req = $db->prepare("SELECT Prenom_Personne, Nom_Personne, Role_Personne FROM utilisateur WHERE MAIL_Personne=:mail and MDP_Personne=:mdp");
...
if (count($resultat) == 0) {
    header("location: Page-co.php?mes=inc");
} else {
